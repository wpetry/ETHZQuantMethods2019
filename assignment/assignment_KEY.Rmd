...
```{r, include=T}
library(popdemo)
lapply(goteborg, eigs, what = "lambda")
lapply(tjarno, eigs, what = "lambda")
```
...
```{r, include=T}
lapply(lapply(goteborg, sens), function(x) which(x == max(x), arr.ind = TRUE))
lapply(lapply(tjarno, sens), function(x) which(x == max(x), arr.ind = TRUE))
```
...
```{r iid, include=T}
Eiid <- matrix(1/3, nrow = 3, ncol = 3,
               dimnames = list(c("mild", "normal", "severe"),
                               c("mild", "normal", "severe")))
```
...
```{r, include=T}
stoch(tjarno, "lambda", Aseq = Eiid, vector = matrix(c(10, 10, 10, 10, 10), ncol = 1), iterations = 1000, discard = 100)
```
...
```{r, include=T}
round(table(seaice$ice)/nrow(seaice), 2)
```

> mild is underrepresented, normal is overrepresented, and severe is almost identical
...
> A positive autocorrelation implies "sticky" environmental conditions. A given ice year type is more likely than chance to persist the next year.
...
```{r, include=T}
stoch(tjarno, what = "lambda", Aseq = env_model, iterations = 1000, discard = 100)
```
...
> The decline in sea ice should have a positive effect on lambda given that "normal" and "severe" sea ice years have lower lambdas than "mild" years in both populations.
